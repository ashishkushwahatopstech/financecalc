...
function updateNavbarUI(user) {
  ...
  authContainer.innerHTML = `
    ...
    <div class="hidden lg:flex items-center gap-2">
      ...
      ` : `
        <button id="btn-login-nav" class="inline-flex items-center gap-2 px-4 py-2 bg-slate-900 hover:bg-slate-800 text-white rounded-xl text-xs font-semibold shadow-xs transition-all transform active:scale-95 cursor-pointer whitespace-nowrap">
          <svg class="w-4 h-4 shrink-0" viewBox="0 0 24 24">
            <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
            <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
            <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.06H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.94l2.85-2.22.81-.63z"/>
...
if (typeof window !== 'undefined') {
  ...
  function initTheme() {
    const currentTheme = localStorage.getItem('theme') || 'dark'; // Dark mode default as requested
  ...
  // Dynamically inject green abacus vector SVG favicon site-wide & theme switcher
  window.addEventListener('DOMContentLoaded', () => {
    ...
    // Inject theme switcher button dynamically
    injectThemeToggle();
  });
}
...
function injectThemeToggle() {
  if (document.getElementById('admin-theme-toggle-btn')) return;

  const btn = document.createElement('button');
  btn.id = 'admin-theme-toggle-btn';
  btn.className = 'fixed bottom-6 right-6 z-[99999] w-10 h-10 rounded-full flex items-center justify-center bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-800 dark:text-slate-100 shadow-sm cursor-pointer hover:scale-105 active:scale-95 transition-all focus:outline-none';
  btn.setAttribute('aria-label', 'Toggle Light/Dark Theme');
  
  const getSunIcon = () => `<svg class="w-5 h-5 text-amber-500" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364-6.364l-.707.707M6.343 17.657l-.707.707m12.728 0l-.707-.707M6.343 6.343l-.707-.707M14 12a2 2 0 11-4 0 2 2 0 014 0z"></path></svg>`;
  const getMoonIcon = () => `<svg class="w-5 h-5 text-indigo-500" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z"></path></svg>`;

  btn.innerHTML = document.documentElement.classList.contains('dark') ? getSunIcon() : getMoonIcon();
